...
document.addEventListener( 'DOMContentLoaded', function (){
    // theme switcher
    var switchDN = document.querySelector('#theme');
    var html = document.querySelector('html');
    var switchLight = document.querySelector('.theme-light');
    var switchDark = document.querySelector('.theme-dark');
    var currentTheme = localStorage.getItem('vdn-theme');

    if ( currentTheme == "dark" ){
        switchDN.checked = true;
    } else {
        switchDN.checked = false;
    }
    // console.log(switchDN.checked);
    switchDN.addEventListener('change', function(){
        // console.log(theme);
        if ( this.checked ) {
            html.setAttribute('data-theme', 'dark');
            localStorage.setItem('vdn-theme', 'dark');
        } else {
            html.setAttribute('data-theme', 'light');
            localStorage.setItem('vdn-theme', 'light');
        }
    });
    ...
    // image slider
    const swImage = new Swiper('.swiper-image', {
        effect: 'slide',
        loop: false,
    });
    const swText = new Swiper('.swiper-text', {
        effect: 'fade',
        loop: false,
    });

    var btnSlides = document.querySelectorAll('.btn-slide');
    btnSlides.forEach(function(el){
        el.addEventListener('mouseenter', function(){
            var index = el.getAttribute('data-target');

            btnSlides.forEach(function(e){
                e.classList.remove('open');
            });
            el.classList.add('open');

            swImage.slideTo(index);
            swText.slideTo(index);
        });
    });
    
    
    // how media slider
    const howMedia = new Swiper('.swiper-how-media', {
        effect: 'slide',
        loop: false,
    });
    
    var itemSlides = document.querySelectorAll('#accHow h3.btn');
    itemSlides.forEach(function(el){
        el.addEventListener('click', function(){
            var index = el.getAttribute('data-slide');
            howMedia.slideTo(index);
        });
    });
    ...
    (function() {
        ...
        var ftElement = document.querySelector('.swiper-feat');
        ...
        if ( ftElement ){
            ...
            const breakpointChecker = function() {
                ...
                if ( breakpoint.matches === true ) {
                    ...
                    if ( ftSwiper !== undefined )
                    // ftSwiper.destroy(true, true);
                    ftSwiper.forEach(function(el){
            ...
            const enableSwiper = function() {
                ftSwiper = new Swiper ('.swiper-feat', {
                    // loop: true,
    ...
    let clipboard = new ClipboardJS('.btn-copy', {
        target: function(trigger) {
            return trigger.closest('.box-highlight').children[1];
        }
    });
    ...
    // infinite-scroll
    let elemInfinite = document.querySelector('.box-post-pager');
    let infScroll = new InfiniteScroll( elemInfinite, {
        // path: 'page/{{#}}',
        path: '.page-link-next',
        // checkLastPage: '.page-link-next',
        append: '.box-post',
        history: false,
  ...
  const searchBar = document.getElementById("inSearch");
  const placeholderText = "Reverse IP Lookup (e.g. 142.250.80.110)";
  searchBar.focus();

  let currentIndex = 0;
//
  function updatePlaceholder() {
    searchBar.placeholder = placeholderText.substring(0, currentIndex);
    currentIndex = Math.min(placeholderText.length, (currentIndex + 1));
  }

  setInterval(updatePlaceholder, Math.floor(Math.random() * (31)) + 90);

});

document.getElementById("ip-button").addEventListener("click", function() {
      // Get the value from the input field
      var ip = document.getElementById("inSearch").value;
      var url = "https://app.validin.com/reverse-ip";
      if (ip) {
        url = url + "?find=" + ip;
      }
      // Set the href attribute of the anchor element
      document.getElementById("ip-button").setAttribute("href", url);
});
...
document.getElementById("searchIcon").addEventListener("click", function() {
      ...
      // Set the href attribute of the anchor element
      document.getElementById("searchIcon").setAttribute("href", url);
});
